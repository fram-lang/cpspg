...
module States = struct
  ...
  *)
  let rec state_0 ~_loc _c0_grammar_starting = 
    let rec _c1_decl ~_loc x = state_30 ~_loc x _c2_list
    and _c2_list ~_loc x = state_32 ~_loc x _c0_grammar_starting in
    ...
    | DCODE x ->
      let _, _l = shift () in
      let _loc = loc_shift ~_loc _l in
      state_1 ~_loc x _c1_decl
    (* Shift *)
    | DTOKEN ->
      let _, _l = shift () in
      let _loc = loc_shift ~_loc _l in
      state_2 ~_loc _c1_decl
    (* Shift *)
    | DTYPE ->
      let _, _l = shift () in
      let _loc = loc_shift ~_loc _l in
      state_8 ~_loc _c1_decl
    (* Shift *)
    | DSTART ->
      let _, _l = shift () in
      let _loc = loc_shift ~_loc _l in
      state_15 ~_loc _c1_decl
    (* Shift *)
    | DLEFT ->
      let _, _l = shift () in
      let _loc = loc_shift ~_loc _l in
      state_20 ~_loc _c1_decl
    (* Shift *)
    | DRIGHT ->
      let _, _l = shift () in
      let _loc = loc_shift ~_loc _l in
      state_26 ~_loc _c1_decl
    (* Shift *)
    | DNONASSOC ->
      let _, _l = shift () in
      let _loc = loc_shift ~_loc _l in
      state_28 ~_loc _c1_decl
    | _ ->
      fail [ "DCODE"; "DTOKEN"; "DTYPE"; "DSTART"; "DLEFT"; "DRIGHT"; "DNONASSOC"; "DSEP" ]
  ...
  *)
  and state_1 ~_loc a0_DCODE _c0_decl = 
    match lookahead () with
    (* Reduce *)
  ...
  *)
  and state_2 ~_loc _c0_decl = 
    let rec _c1_option ~_loc x = state_4 ~_loc x _c0_decl in
    match lookahead () with
    (* Shift *)
    | TYPE x ->
      let _, _l = shift () in
      let _loc = loc_shift ~_loc _l in
      state_3 ~_loc x _c1_option
    (* Reduce *)
    | TID _ | DCODE _ | DTOKEN | DTYPE | DSTART | DLEFT | DRIGHT | DNONASSOC | DSEP ->
      ...
      _c1_option ~_loc x
    | _ ->
      fail [ "TID"; "TYPE"; "DCODE"; "DTOKEN"; "DTYPE"; "DSTART"; "DLEFT"; "DRIGHT"; "DNONASSOC"; "DSEP" ]
  ...
  *)
  and state_3 ~_loc a0_TYPE _c0_option = 
    match lookahead () with
    (* Reduce *)
    | ID _ | TID _ | DCODE _ | DTOKEN | DTYPE | DSTART | DLEFT | DRIGHT | DNONASSOC | DSEP ->
      ...
      _c0_option ~_loc x
    | _ ->
      fail [ "ID"; "TID"; "DCODE"; "DTOKEN"; "DTYPE"; "DSTART"; "DLEFT"; "DRIGHT"; "DNONASSOC"; "DSEP" ]
  ...
  *)
  and state_4 ~_loc a0_option _c0_decl = 
    let rec _c1_list ~_loc x = state_7 ~_loc x a0_option _c0_decl in
    match lookahead () with
    (* Shift *)
    | TID x ->
      let _, _l = shift () in
      let _loc = loc_shift ~_loc _l in
      state_5 ~_loc x _c1_list
    (* Reduce *)
    | DCODE _ | DTOKEN | DTYPE | DSTART | DLEFT | DRIGHT | DNONASSOC | DSEP ->
      let _loc = loc_reduce ~_loc 0
      and x = Actions.a6 ~_loc () in
      _c1_list ~_loc x
    | _ ->
      fail [ "TID"; "DCODE"; "DTOKEN"; "DTYPE"; "DSTART"; "DLEFT"; "DRIGHT"; "DNONASSOC"; "DSEP" ]
  ...
  (*
    ...
    ACTION:
      ...
      DCODE DTOKEN DTYPE DSTART DLEFT DRIGHT DNONASSOC DSEP -> reduce 1 1
  *)
  and state_5 ~_loc a0_TID _c0_list = 
    let rec _c1_list ~_loc x = state_6 ~_loc x a0_TID _c0_list in
    match lookahead () with
    (* Shift *)
    | TID x ->
      let _, _l = shift () in
  ...
  (*
    ...
    ACTION:
      DCODE DTOKEN DTYPE DSTART DLEFT DRIGHT DNONASSOC DSEP -> reduce 0 0
  *)
  and state_6 ~_loc a0_list a1_TID _c0_list = 
    match lookahead () with
    (* Reduce *)
    | DCODE _ | DTOKEN | DTYPE | DSTART | DLEFT | DRIGHT | DNONASSOC | DSEP ->
  ...
  (*
    ...
    ACTION:
      DCODE DTOKEN DTYPE DSTART DLEFT DRIGHT DNONASSOC DSEP -> reduce 0 0
  *)
  and state_7 ~_loc a0_list a1_option _c0_decl = 
    match lookahead () with
    (* Reduce *)
    | DCODE _ | DTOKEN | DTYPE | DSTART | DLEFT | DRIGHT | DNONASSOC | DSEP ->
  ...
  (*
    ...
    ACTION:
      TYPE -> shift
  *)
  and state_8 ~_loc _c0_decl = 
    match lookahead () with
    (* Shift *)
    | TYPE x ->
      let _, _l = shift () in
      let _loc = loc_shift ~_loc _l in
      state_9 ~_loc x _c0_decl
    | _ ->
      fail [ "TYPE" ]
  ...
  *)
  and state_9 ~_loc a0_TYPE _c0_decl = 
    let rec _c1_symbol ~_loc x = state_12 ~_loc x _c2_list
    and _c2_list ~_loc x = state_14 ~_loc x a0_TYPE _c0_decl in
    match lookahead () with
    (* Shift *)
    | ID x ->
      let _, _l = shift () in
      let _loc = loc_shift ~_loc _l in
      state_10 ~_loc x _c1_symbol
    (* Shift *)
    | TID x ->
      let _, _l = shift () in
      let _loc = loc_shift ~_loc _l in
      state_11 ~_loc x _c1_symbol
    (* Reduce *)
    | DCODE _ | DTOKEN | DTYPE | DSTART | DLEFT | DRIGHT | DNONASSOC | DSEP ->
      let _loc = loc_reduce ~_loc 0
      and x = Actions.a6 ~_loc () in
  ...
  *)
  and state_10 ~_loc a0_ID _c0_symbol = 
    match lookahead () with
    (* Reduce *)
    | ID _ | TID _ | CODE _ | DCODE _ | DTOKEN | DTYPE | DSTART | DLEFT | DRIGHT | DNONASSOC | DSEP | DWHEN | DPREC | COMMA | EQ | PLUS | QMARK | SEMI | STAR | LPAREN | RPAREN ->
      ...
      _c0_symbol ~_loc x
    | _ ->
      fail [ "ID"; "TID"; "CODE"; "DCODE"; "DTOKEN"; "DTYPE"; "DSTART"; "DLEFT"; "DRIGHT"; "DNONASSOC"; "DSEP"; "DWHEN"; "DPREC"; "COMMA"; "EQ"; "PLUS"; "QMARK"; "SEMI"; "STAR"; "LPAREN"; "RPAREN" ]
  ...
  (*
    ...
    ACTION:
      ID TID CODE DCODE DTOKEN DTYPE DSTART DLEFT DRIGHT DNONASSOC DSEP DWHEN DPREC COMMA EQ PLUS QMARK SEMI STAR LPAREN RPAREN -> reduce 0 0
  *)
  and state_11 ~_loc a0_TID _c0_symbol = 
    match lookahead () with
    (* Reduce *)
    | ID _ | TID _ | CODE _ | DCODE _ | DTOKEN | DTYPE | DSTART | DLEFT | DRIGHT | DNONASSOC | DSEP | DWHEN | DPREC | COMMA | EQ | PLUS | QMARK | SEMI | STAR | LPAREN | RPAREN ->
      let _loc = loc_reduce ~_loc 1
  ...
  (*
    ...
    ACTION:
      ...
      DCODE DTOKEN DTYPE DSTART DLEFT DRIGHT DNONASSOC DSEP -> reduce 2 1
  *)
  and state_12 ~_loc a0_symbol _c0_list = 
    let rec _c1_symbol ~_loc x = state_12 ~_loc x _c2_list
    and _c2_list ~_loc x = state_13 ~_loc x a0_symbol _c0_list in
    match lookahead () with
    (* Shift *)
    | ID x ->
      let _, _l = shift () in
  ...
  (*
    ...
    ACTION:
      DCODE DTOKEN DTYPE DSTART DLEFT DRIGHT DNONASSOC DSEP -> reduce 0 0
  *)
  and state_13 ~_loc a0_list a1_symbol _c0_list = 
    match lookahead () with
    (* Reduce *)
    | DCODE _ | DTOKEN | DTYPE | DSTART | DLEFT | DRIGHT | DNONASSOC | DSEP ->
      let _loc = loc_reduce ~_loc 2
  ...
  (*
    ...
    ACTION:
      DCODE DTOKEN DTYPE DSTART DLEFT DRIGHT DNONASSOC DSEP -> reduce 0 0
  *)
  and state_14 ~_loc a0_list a1_TYPE _c0_decl = 
    match lookahead () with
    (* Reduce *)
    | DCODE _ | DTOKEN | DTYPE | DSTART | DLEFT | DRIGHT | DNONASSOC | DSEP ->
      let _loc = loc_reduce ~_loc 3
  ...
  *)
  and state_15 ~_loc _c0_decl = 
    let rec _c1_option ~_loc x = state_16 ~_loc x _c0_decl in
    match lookahead () with
    (* Shift *)
    | TYPE x ->
      let _, _l = shift () in
    ...
    | ID _ | DCODE _ | DTOKEN | DTYPE | DSTART | DLEFT | DRIGHT | DNONASSOC | DSEP ->
      ...
      _c1_option ~_loc x
    | _ ->
      fail [ "ID"; "TYPE"; "DCODE"; "DTOKEN"; "DTYPE"; "DSTART"; "DLEFT"; "DRIGHT"; "DNONASSOC"; "DSEP" ]
  ...
  (*
    ...
    ACTION:
      ...
      DCODE DTOKEN DTYPE DSTART DLEFT DRIGHT DNONASSOC DSEP -> reduce 1 1
  *)
  and state_16 ~_loc a0_option _c0_decl = 
    let rec _c1_list ~_loc x = state_19 ~_loc x a0_option _c0_decl in
    match lookahead () with
    (* Shift *)
    | ID x ->
      let _, _l = shift () in
      let _loc = loc_shift ~_loc _l in
      state_17 ~_loc x _c1_list
    (* Reduce *)
    | DCODE _ | DTOKEN | DTYPE | DSTART | DLEFT | DRIGHT | DNONASSOC | DSEP ->
      let _loc = loc_reduce ~_loc 0
      and x = Actions.a6 ~_loc () in
      _c1_list ~_loc x
    | _ ->
      fail [ "ID"; "DCODE"; "DTOKEN"; "DTYPE"; "DSTART"; "DLEFT"; "DRIGHT"; "DNONASSOC"; "DSEP" ]
  ...
  (*
    ...
    ACTION:
      ...
      DCODE DTOKEN DTYPE DSTART DLEFT DRIGHT DNONASSOC DSEP -> reduce 1 1
  *)
  and state_17 ~_loc a0_ID _c0_list = 
    let rec _c1_list ~_loc x = state_18 ~_loc x a0_ID _c0_list in
    match lookahead () with
    (* Shift *)
    | ID x ->
      let _, _l = shift () in
  ...
  (*
    ...
    ACTION:
      DCODE DTOKEN DTYPE DSTART DLEFT DRIGHT DNONASSOC DSEP -> reduce 0 0
  *)
  and state_18 ~_loc a0_list a1_ID _c0_list = 
    match lookahead () with
    (* Reduce *)
    | DCODE _ | DTOKEN | DTYPE | DSTART | DLEFT | DRIGHT | DNONASSOC | DSEP ->
      let _loc = loc_reduce ~_loc 2
  ...
  (*
    ...
    ACTION:
      DCODE DTOKEN DTYPE DSTART DLEFT DRIGHT DNONASSOC DSEP -> reduce 0 0
  *)
  and state_19 ~_loc a0_list a1_option _c0_decl = 
    match lookahead () with
    (* Reduce *)
    | DCODE _ | DTOKEN | DTYPE | DSTART | DLEFT | DRIGHT | DNONASSOC | DSEP ->
      let _loc = loc_reduce ~_loc 3
  ...
  (*
    ...
    ACTION:
      ...
      DCODE DTOKEN DTYPE DSTART DLEFT DRIGHT DNONASSOC DSEP -> reduce 2 1
  *)
  and state_20 ~_loc _c0_decl = 
    let rec _c1_ident ~_loc x = state_23 ~_loc x _c2_list
    and _c2_list ~_loc x = state_25 ~_loc x _c0_decl in
    match lookahead () with
    (* Shift *)
    | ID x ->
      let _, _l = shift () in
      let _loc = loc_shift ~_loc _l in
      state_21 ~_loc x _c1_ident
    (* Shift *)
    | TID x ->
      let _, _l = shift () in
      let _loc = loc_shift ~_loc _l in
      state_22 ~_loc x _c1_ident
    (* Reduce *)
    | DCODE _ | DTOKEN | DTYPE | DSTART | DLEFT | DRIGHT | DNONASSOC | DSEP ->
      let _loc = loc_reduce ~_loc 0
      and x = Actions.a6 ~_loc () in
  ...
  *)
  and state_21 ~_loc a0_ID _c0_ident = 
    match lookahead () with
    (* Reduce *)
    | ID _ | TID _ | CODE _ | DCODE _ | DTOKEN | DTYPE | DSTART | DLEFT | DRIGHT | DNONASSOC | DSEP | DWHEN ->
      ...
      _c0_ident ~_loc x
    | _ ->
      fail [ "ID"; "TID"; "CODE"; "DCODE"; "DTOKEN"; "DTYPE"; "DSTART"; "DLEFT"; "DRIGHT"; "DNONASSOC"; "DSEP"; "DWHEN" ]
  ...
  (*
    ...
    ACTION:
      ID TID CODE DCODE DTOKEN DTYPE DSTART DLEFT DRIGHT DNONASSOC DSEP DWHEN -> reduce 0 0
  *)
  and state_22 ~_loc a0_TID _c0_ident = 
    match lookahead () with
    (* Reduce *)
    | ID _ | TID _ | CODE _ | DCODE _ | DTOKEN | DTYPE | DSTART | DLEFT | DRIGHT | DNONASSOC | DSEP | DWHEN ->
      let _loc = loc_reduce ~_loc 1
  ...
  (*
    ...
    ACTION:
      ...
      DCODE DTOKEN DTYPE DSTART DLEFT DRIGHT DNONASSOC DSEP -> reduce 2 1
  *)
  and state_23 ~_loc a0_ident _c0_list = 
    let rec _c1_ident ~_loc x = state_23 ~_loc x _c2_list
    and _c2_list ~_loc x = state_24 ~_loc x a0_ident _c0_list in
    match lookahead () with
    (* Shift *)
    | ID x ->
      let _, _l = shift () in
  ...
  (*
    ...
    ACTION:
      DCODE DTOKEN DTYPE DSTART DLEFT DRIGHT DNONASSOC DSEP -> reduce 0 0
  *)
  and state_24 ~_loc a0_list a1_ident _c0_list = 
    match lookahead () with
    (* Reduce *)
    | DCODE _ | DTOKEN | DTYPE | DSTART | DLEFT | DRIGHT | DNONASSOC | DSEP ->
      let _loc = loc_reduce ~_loc 2
  ...
  (*
    ...
    ACTION:
      DCODE DTOKEN DTYPE DSTART DLEFT DRIGHT DNONASSOC DSEP -> reduce 0 0
  *)
  and state_25 ~_loc a0_list _c0_decl = 
    match lookahead () with
    (* Reduce *)
    | DCODE _ | DTOKEN | DTYPE | DSTART | DLEFT | DRIGHT | DNONASSOC | DSEP ->
      let _loc = loc_reduce ~_loc 2
  ...
  (*
    ...
    ACTION:
      ...
      DCODE DTOKEN DTYPE DSTART DLEFT DRIGHT DNONASSOC DSEP -> reduce 2 1
  *)
  and state_26 ~_loc _c0_decl = 
    let rec _c1_ident ~_loc x = state_23 ~_loc x _c2_list
    and _c2_list ~_loc x = state_27 ~_loc x _c0_decl in
    match lookahead () with
    (* Shift *)
    | ID x ->
      let _, _l = shift () in
  ...
  (*
    ...
    ACTION:
      DCODE DTOKEN DTYPE DSTART DLEFT DRIGHT DNONASSOC DSEP -> reduce 0 0
  *)
  and state_27 ~_loc a0_list _c0_decl = 
    match lookahead () with
    (* Reduce *)
    | DCODE _ | DTOKEN | DTYPE | DSTART | DLEFT | DRIGHT | DNONASSOC | DSEP ->
      let _loc = loc_reduce ~_loc 2
  ...
  (*
    ...
    ACTION:
      ...
      DCODE DTOKEN DTYPE DSTART DLEFT DRIGHT DNONASSOC DSEP -> reduce 2 1
  *)
  and state_28 ~_loc _c0_decl = 
    let rec _c1_ident ~_loc x = state_23 ~_loc x _c2_list
    and _c2_list ~_loc x = state_29 ~_loc x _c0_decl in
    match lookahead () with
    (* Shift *)
    | ID x ->
      let _, _l = shift () in
  ...
  (*
    ...
    ACTION:
      DCODE DTOKEN DTYPE DSTART DLEFT DRIGHT DNONASSOC DSEP -> reduce 0 0
  *)
  and state_29 ~_loc a0_list _c0_decl = 
    match lookahead () with
    (* Reduce *)
    | DCODE _ | DTOKEN | DTYPE | DSTART | DLEFT | DRIGHT | DNONASSOC | DSEP ->
      let _loc = loc_reduce ~_loc 2
  ...
  (*
    ...
    ACTION:
      ...
      DCODE DTOKEN DTYPE DSTART DLEFT DRIGHT DNONASSOC -> shift
  *)
  and state_30 ~_loc a0_decl _c0_list = 
    let rec _c1_decl ~_loc x = state_30 ~_loc x _c2_list
    and _c2_list ~_loc x = state_31 ~_loc x a0_decl _c0_list in
    match lookahead () with
    (* Reduce *)
    | DSEP ->
      let _loc = loc_reduce ~_loc 0
  ...
  (*
    ...
    ACTION:
      DSEP -> reduce 0 0
  *)
  and state_31 ~_loc a0_list a1_decl _c0_list = 
    match lookahead () with
    (* Reduce *)
    | DSEP ->
      let _loc = loc_reduce ~_loc 2
  ...
  (*
    ...
    ACTION:
      DSEP -> shift
  *)
  and state_32 ~_loc a0_list _c0_grammar_starting = 
    match lookahead () with
    (* Shift *)
    | DSEP ->
      let _, _l = shift () in
      let _loc = loc_shift ~_loc _l in
      state_33 ~_loc a0_list _c0_grammar_starting
    | _ ->
      fail [ "DSEP" ]
  ...
  (*
    ...
    ACTION:
      ...
      EOF -> reduce 3 1
  *)
  and state_33 ~_loc a1_list _c0_grammar_starting = 
    let rec _c1_rule ~_loc x = state_35 ~_loc x _c3_list
    and _c2_boption ~_loc x = state_36 ~_loc x _c1_rule
    and _c3_list ~_loc x = state_98 ~_loc x a1_list _c0_grammar_starting in
    match lookahead () with
    (* Reduce *)
    | ID _ ->
      let _loc = loc_reduce ~_loc 0
  ...
  *)
  ...
  (*
    ...
    ACTION:
      ...
      EOF -> reduce 3 1
  *)
  and state_35 ~_loc a0_rule _c0_list = 
    let rec _c1_rule ~_loc x = state_35 ~_loc x _c3_list
    and _c2_boption ~_loc x = state_36 ~_loc x _c1_rule
    and _c3_list ~_loc x = state_97 ~_loc x a0_rule _c0_list in
    match lookahead () with
    (* Reduce *)
    | ID _ ->
      let _loc = loc_reduce ~_loc 0
  ...
  (*
    ...
    ACTION:
      ID -> shift
  *)
  and state_36 ~_loc a0_boption _c0_rule = 
    match lookahead () with
    (* Shift *)
    | ID x ->
      let _, _l = shift () in
      let _loc = loc_shift ~_loc _l in
      state_37 ~_loc x a0_boption _c0_rule
    | _ ->
      fail [ "ID" ]
  ...
  (*
    ...
    ACTION:
      ...
      COLON -> reduce 2 1
  *)
  and state_37 ~_loc a0_ID a1_boption _c0_rule = 
    let rec _c1_parameters ~_loc x = state_45 ~_loc x _c2_loption
    and _c2_loption ~_loc x = state_46 ~_loc x a0_ID a1_boption _c0_rule in
    match lookahead () with
    (* Shift *)
    | LPAREN ->
      let _, _l = shift () in
      let _loc = loc_shift ~_loc _l in
      state_38 ~_loc _c1_parameters
    (* Reduce *)
    | COLON ->
      let _loc = loc_reduce ~_loc 0
  ...
  (*
    ...
    ACTION:
      ...
      RPAREN -> reduce 3 1
  *)
  and state_38 ~_loc _c0_parameters = 
    let rec _c1_symbol ~_loc x = state_39 ~_loc x _c2_separated_nonempty_list
    and _c2_separated_nonempty_list ~_loc x = state_42 ~_loc x _c3_loption
    and _c3_loption ~_loc x = state_43 ~_loc x _c0_parameters in
    match lookahead () with
    (* Shift *)
    | ID x ->
      let _, _l = shift () in
  ...
  (*
    ...
    ACTION:
      ...
      RPAREN -> reduce 0 1
  *)
  and state_39 ~_loc a0_symbol _c0_separated_nonempty_list = 
    match lookahead () with
    (* Shift *)
    | COMMA ->
      let _, _l = shift () in
      let _loc = loc_shift ~_loc _l in
      state_40 ~_loc a0_symbol _c0_separated_nonempty_list
    (* Reduce *)
    | RPAREN ->
      let _loc = loc_reduce ~_loc 1
      and x = Actions.a22 ~_loc (Actions.a21 ~_loc a0_symbol ()) () in
      _c0_separated_nonempty_list ~_loc x
    | _ ->
      fail [ "COMMA"; "RPAREN" ]
  ...
  (*
    ...
    ACTION:
      ID TID -> shift
  *)
  and state_40 ~_loc a1_symbol _c0_separated_nonempty_list = 
    let rec _c1_symbol ~_loc x = state_39 ~_loc x _c2_separated_nonempty_list
    and _c2_separated_nonempty_list ~_loc x = state_41 ~_loc x a1_symbol _c0_separated_nonempty_list in
    match lookahead () with
    (* Shift *)
    | ID x ->
      let _, _l = shift () in
  ...
  (*
    ...
    ACTION:
      RPAREN -> reduce 0 0
  *)
  and state_41 ~_loc a0_separated_nonempty_list a2_symbol _c0_separated_nonempty_list = 
    match lookahead () with
    (* Reduce *)
    | RPAREN ->
      let _loc = loc_reduce ~_loc 3
  ...
  (*
    ...
    ACTION:
      RPAREN -> reduce 0 0
  *)
  and state_42 ~_loc a0_separated_nonempty_list _c0_loption = 
    match lookahead () with
    (* Reduce *)
    | RPAREN ->
      let _loc = loc_reduce ~_loc 1
  ...
  (*
    ...
    ACTION:
      RPAREN -> shift
  *)
  and state_43 ~_loc a0_loption _c0_parameters = 
    match lookahead () with
    (* Shift *)
    | RPAREN ->
      let _, _l = shift () in
      let _loc = loc_shift ~_loc _l in
      state_44 ~_loc a0_loption _c0_parameters
    | _ ->
      fail [ "RPAREN" ]
  ...
  (*
    ...
    ACTION:
      COLON -> reduce 0 0
  *)
  and state_44 ~_loc a1_loption _c0_parameters = 
    match lookahead () with
    (* Reduce *)
    | COLON ->
      let _loc = loc_reduce ~_loc 3
      and x = Actions.a27 ~_loc () (Actions.a26 ~_loc a1_loption ()) () () in
      _c0_parameters ~_loc x
    | _ ->
      fail [ "COLON" ]
  ...
  (*
    ...
    ACTION:
      COLON -> reduce 0 0
  *)
  and state_45 ~_loc a0_parameters _c0_loption = 
    match lookahead () with
    (* Reduce *)
    | COLON ->
      let _loc = loc_reduce ~_loc 1
  ...
  (*
    ...
    ACTION:
      COLON -> shift
  *)
  and state_46 ~_loc a0_loption a1_ID a2_boption _c0_rule = 
    match lookahead () with
    (* Shift *)
    | COLON ->
      let _, _l = shift () in
      let _loc = loc_shift ~_loc _l in
      state_47 ~_loc a0_loption a1_ID a2_boption _c0_rule
    | _ ->
      fail [ "COLON" ]
  ...
  *)
  and state_47 ~_loc a1_loption a2_ID a3_boption _c0_rule = 
    let rec _c1_option ~_loc x = state_49 ~_loc x a1_loption a2_ID a3_boption _c0_rule in
    match lookahead () with
    (* Shift *)
    | BAR ->
      let _, _l = shift () in
      let _loc = loc_shift ~_loc _l in
      state_48 ~_loc _c1_option
    (* Reduce *)
    | ID _ | TID _ | CODE _ | DWHEN | DPREC ->
      ...
      _c1_option ~_loc x
    | _ ->
      fail [ "ID"; "TID"; "CODE"; "DWHEN"; "DPREC"; "BAR" ]
  ...
  *)
  ...
  (*
    ...
    ACTION:
      ...
      ID TID -> shift
  *)
  and state_49 ~_loc a0_option a2_loption a3_ID a4_boption _c0_rule = 
    let rec _c1_symbol ~_loc x = state_52 ~_loc x _c4_actual
    and _c2_production ~_loc x = state_76 ~_loc x _c6_separated_nonempty_list
    and _c3_producer ~_loc x = state_54 ~_loc x _c5_list
    and _c4_actual ~_loc x = state_55 ~_loc x _c3_producer _c4_actual
    and _c5_list ~_loc x = state_78 ~_loc x _c2_production
    and _c6_separated_nonempty_list ~_loc x = state_95 ~_loc x a0_option a2_loption a3_ID a4_boption _c0_rule in
    match lookahead () with
    (* Reduce *)
    | CODE _ | DWHEN | DPREC ->
      let _loc = loc_reduce ~_loc 0
    ...
    | ID x ->
      let _, _l = shift () in
      let _loc = loc_shift ~_loc _l in
      state_50 ~_loc x _c1_symbol _c3_producer
    (* Shift *)
    | TID x ->
      let _, _l = shift () in
      let _loc = loc_shift ~_loc _l in
  ...
  (*
    ...
    ACTION:
      ...
      EQ -> shift
  *)
  and state_50 ~_loc a0_ID _c0_symbol _c1_producer = 
    match lookahead () with
    (* Reduce *)
    ...
    | EQ ->
      let _, _l = shift () in
      let _loc = loc_shift ~_loc _l in
      state_51 ~_loc a0_ID _c1_producer
    | _ ->
      fail [ "ID"; "TID"; "CODE"; "DWHEN"; "DPREC"; "COMMA"; "EQ"; "PLUS"; "QMARK"; "SEMI"; "STAR"; "LPAREN"; "RPAREN" ]
  ...
  (*
    ...
    ACTION:
      ID TID -> shift
  *)
  and state_51 ~_loc a1_ID _c0_producer = 
    let rec _c1_symbol ~_loc x = state_52 ~_loc x _c2_actual
    and _c2_actual ~_loc x = state_74 ~_loc x a1_ID _c0_producer _c2_actual in
    match lookahead () with
    (* Shift *)
    | ID x ->
      let _, _l = shift () in
  ...
  *)
  and state_52 ~_loc a0_symbol _c0_actual = 
    let rec _c1_args ~_loc x = state_72 ~_loc x _c2_loption
    and _c2_loption ~_loc x = state_73 ~_loc x a0_symbol _c0_actual in
    match lookahead () with
    (* Shift *)
    | LPAREN ->
      let _, _l = shift () in
      let _loc = loc_shift ~_loc _l in
      state_53 ~_loc _c1_args
    (* Reduce *)
    | ID _ | TID _ | CODE _ | DWHEN | DPREC | COMMA | PLUS | QMARK | SEMI | STAR | RPAREN ->
      ...
      _c2_loption ~_loc x
    | _ ->
      fail [ "ID"; "TID"; "CODE"; "DWHEN"; "DPREC"; "COMMA"; "PLUS"; "QMARK"; "SEMI"; "STAR"; "LPAREN"; "RPAREN" ]
  ...
  (*
    ...
    ACTION:
      ...
      ID TID -> shift
  *)
  and state_53 ~_loc _c0_args = 
    let rec _c1_symbol ~_loc x = state_52 ~_loc x _c3_actual
    and _c2_producer ~_loc x = state_54 ~_loc x _c5_list
    and _c3_actual ~_loc x = state_64 ~_loc x _c2_producer _c3_actual _c4_arg
    and _c4_arg ~_loc x = state_65 ~_loc x _c6_separated_nonempty_list
    and _c5_list ~_loc x = state_67 ~_loc x _c4_arg
    and _c6_separated_nonempty_list ~_loc x = state_70 ~_loc x _c0_args in
    match lookahead () with
    (* Reduce *)
    | CODE _ ->
      let _loc = loc_reduce ~_loc 0
    ...
    | ID x ->
      let _, _l = shift () in
      let _loc = loc_shift ~_loc _l in
      state_50 ~_loc x _c1_symbol _c2_producer
    (* Shift *)
    | TID x ->
      let _, _l = shift () in
      let _loc = loc_shift ~_loc _l in
  ...
  (*
    ...
    ACTION:
      ...
      ID TID -> shift
  *)
  and state_54 ~_loc a0_producer _c0_list = 
    let rec _c1_symbol ~_loc x = state_52 ~_loc x _c3_actual
    and _c2_producer ~_loc x = state_54 ~_loc x _c4_list
    and _c3_actual ~_loc x = state_55 ~_loc x _c2_producer _c3_actual
    and _c4_list ~_loc x = state_63 ~_loc x a0_producer _c0_list in
    match lookahead () with
    (* Reduce *)
    | CODE _ | DWHEN | DPREC ->
      let _loc = loc_reduce ~_loc 0
  ...
  *)
  and state_55 ~_loc a0_actual _c0_producer _c1_actual = 
    let rec _c2_shorthand ~_loc x = state_61 ~_loc x a0_actual _c1_actual
    and _c3_list ~_loc x = state_62 ~_loc x a0_actual _c0_producer in
    match lookahead () with
    (* Shift *)
    | PLUS ->
      let _, _l = shift () in
      let _loc = loc_shift ~_loc _l in
      state_56 ~_loc _c2_shorthand
    (* Shift *)
    | QMARK ->
      let _, _l = shift () in
      let _loc = loc_shift ~_loc _l in
      state_57 ~_loc _c2_shorthand
    (* Shift *)
    | SEMI ->
      let _, _l = shift () in
      let _loc = loc_shift ~_loc _l in
      state_58 ~_loc _c3_list
    (* Shift *)
    | STAR ->
      let _, _l = shift () in
      let _loc = loc_shift ~_loc _l in
      state_60 ~_loc _c2_shorthand
    (* Reduce *)
    | ID _ | TID _ | CODE _ | DWHEN | DPREC ->
      let _loc = loc_reduce ~_loc 0
      and x = Actions.a6 ~_loc () in
      _c3_list ~_loc x
    | _ ->
      fail [ "ID"; "TID"; "CODE"; "DWHEN"; "DPREC"; "PLUS"; "QMARK"; "SEMI"; "STAR" ]
  ...
  *)
  and state_56 ~_loc _c0_shorthand = 
    match lookahead () with
    (* Reduce *)
    | ID _ | TID _ | CODE _ | DWHEN | DPREC | COMMA | PLUS | QMARK | SEMI | STAR | RPAREN ->
      let _loc = loc_reduce ~_loc 1
      and x = Actions.a31 ~_loc () () in
      _c0_shorthand ~_loc x
    | _ ->
      fail [ "ID"; "TID"; "CODE"; "DWHEN"; "DPREC"; "COMMA"; "PLUS"; "QMARK"; "SEMI"; "STAR"; "RPAREN" ]
  ...
  (*
    ...
    ACTION:
      ID TID CODE DWHEN DPREC COMMA PLUS QMARK SEMI STAR RPAREN -> reduce 0 0
  *)
  and state_57 ~_loc _c0_shorthand = 
    match lookahead () with
    (* Reduce *)
    | ID _ | TID _ | CODE _ | DWHEN | DPREC | COMMA | PLUS | QMARK | SEMI | STAR | RPAREN ->
      let _loc = loc_reduce ~_loc 1
  ...
  *)
  and state_58 ~_loc _c0_list = 
    let rec _c1_list ~_loc x = state_59 ~_loc x _c0_list in
    match lookahead () with
    (* Shift *)
    | SEMI ->
      let _, _l = shift () in
      let _loc = loc_shift ~_loc _l in
      state_58 ~_loc _c1_list
    (* Reduce *)
    | ID _ | TID _ | CODE _ | DWHEN | DINLINE | DPREC | EOF ->
      ...
      _c1_list ~_loc x
    | _ ->
      fail [ "ID"; "TID"; "CODE"; "DWHEN"; "DINLINE"; "DPREC"; "SEMI"; "EOF" ]
  ...
  *)
  and state_59 ~_loc a0_list _c0_list = 
    match lookahead () with
    (* Reduce *)
    | ID _ | TID _ | CODE _ | DWHEN | DINLINE | DPREC | EOF ->
      let _loc = loc_reduce ~_loc 2
      ...
      _c0_list ~_loc x
    | _ ->
      fail [ "ID"; "TID"; "CODE"; "DWHEN"; "DINLINE"; "DPREC"; "EOF" ]
  ...
  (*
    ...
    ACTION:
      ID TID CODE DWHEN DPREC COMMA PLUS QMARK SEMI STAR RPAREN -> reduce 0 0
  *)
  and state_60 ~_loc _c0_shorthand = 
    match lookahead () with
    (* Reduce *)
    | ID _ | TID _ | CODE _ | DWHEN | DPREC | COMMA | PLUS | QMARK | SEMI | STAR | RPAREN ->
      let _loc = loc_reduce ~_loc 1
  ...
  (*
    ...
    ACTION:
      ID TID CODE DWHEN DPREC COMMA PLUS QMARK SEMI STAR RPAREN -> reduce 0 0
  *)
  and state_61 ~_loc a0_shorthand a1_actual _c0_actual = 
    match lookahead () with
    (* Reduce *)
    | ID _ | TID _ | CODE _ | DWHEN | DPREC | COMMA | PLUS | QMARK | SEMI | STAR | RPAREN ->
      let _loc = loc_reduce ~_loc 2
  ...
  (*
    ...
    ACTION:
      ID TID CODE DWHEN DPREC -> reduce 0 0
  *)
  and state_62 ~_loc a0_list a1_actual _c0_producer = 
    match lookahead () with
    (* Reduce *)
    | ID _ | TID _ | CODE _ | DWHEN | DPREC ->
      let _loc = loc_reduce ~_loc 2
  ...
  *)
  and state_63 ~_loc a0_list a1_producer _c0_list = 
    match lookahead () with
    (* Reduce *)
    | CODE _ | DWHEN | DPREC ->
      let _loc = loc_reduce ~_loc 2
  ...
  *)
  and state_64 ~_loc a0_actual _c0_producer _c1_actual _c2_arg = 
    let rec _c3_shorthand ~_loc x = state_61 ~_loc x a0_actual _c1_actual
    and _c4_list ~_loc x = state_62 ~_loc x a0_actual _c0_producer in
    match lookahead () with
    (* Reduce *)
    | ID _ | TID _ | CODE _ ->
      let _loc = loc_reduce ~_loc 0
    ...
    | PLUS ->
      let _, _l = shift () in
      let _loc = loc_shift ~_loc _l in
      state_56 ~_loc _c3_shorthand
    (* Shift *)
    | QMARK ->
      let _, _l = shift () in
      let _loc = loc_shift ~_loc _l in
      state_57 ~_loc _c3_shorthand
    (* Shift *)
    | SEMI ->
      let _, _l = shift () in
      let _loc = loc_shift ~_loc _l in
      state_58 ~_loc _c4_list
    (* Shift *)
    | STAR ->
      let _, _l = shift () in
      let _loc = loc_shift ~_loc _l in
      state_60 ~_loc _c3_shorthand
    (* Reduce *)
    | COMMA | RPAREN ->
      let _loc = loc_reduce ~_loc 1
      ...
      _c2_arg ~_loc x
    | _ ->
      fail [ "ID"; "TID"; "CODE"; "COMMA"; "PLUS"; "QMARK"; "SEMI"; "STAR"; "RPAREN" ]
  ...
  (*
    ...
    ACTION:
      ...
      RPAREN -> reduce 0 1
  *)
  and state_65 ~_loc a0_arg _c0_separated_nonempty_list = 
    match lookahead () with
    (* Shift *)
    | COMMA ->
      let _, _l = shift () in
      let _loc = loc_shift ~_loc _l in
      state_66 ~_loc a0_arg _c0_separated_nonempty_list
    (* Reduce *)
    | RPAREN ->
      let _loc = loc_reduce ~_loc 1
  ...
  (*
    ...
    ACTION:
      ...
      ID TID -> shift
  *)
  and state_66 ~_loc a1_arg _c0_separated_nonempty_list = 
    let rec _c1_symbol ~_loc x = state_52 ~_loc x _c3_actual
    and _c2_producer ~_loc x = state_54 ~_loc x _c5_list
    and _c3_actual ~_loc x = state_64 ~_loc x _c2_producer _c3_actual _c4_arg
    and _c4_arg ~_loc x = state_65 ~_loc x _c6_separated_nonempty_list
    and _c5_list ~_loc x = state_67 ~_loc x _c4_arg
    and _c6_separated_nonempty_list ~_loc x = state_69 ~_loc x a1_arg _c0_separated_nonempty_list in
    match lookahead () with
    (* Reduce *)
    | CODE _ ->
      let _loc = loc_reduce ~_loc 0
  ...
  (*
    ...
    ACTION:
      CODE -> shift
  *)
  and state_67 ~_loc a0_list _c0_arg = 
    match lookahead () with
    (* Shift *)
    | CODE x ->
      let _, _l = shift () in
      let _loc = loc_shift ~_loc _l in
      state_68 ~_loc x a0_list _c0_arg
    | _ ->
      fail [ "CODE" ]
  ...
  *)
  and state_68 ~_loc a0_CODE a1_list _c0_arg = 
    match lookahead () with
    (* Reduce *)
    | COMMA | RPAREN ->
      let _loc = loc_reduce ~_loc 2
  ...
  (*
    ...
    ACTION:
      RPAREN -> reduce 0 0
  *)
  and state_69 ~_loc a0_separated_nonempty_list a2_arg _c0_separated_nonempty_list = 
    match lookahead () with
    (* Reduce *)
    | RPAREN ->
      let _loc = loc_reduce ~_loc 3
  ...
  (*
    ...
    ACTION:
      RPAREN -> shift
  *)
  and state_70 ~_loc a0_separated_nonempty_list _c0_args = 
    match lookahead () with
    (* Shift *)
    | RPAREN ->
      let _, _l = shift () in
      let _loc = loc_shift ~_loc _l in
      state_71 ~_loc a0_separated_nonempty_list _c0_args
    | _ ->
      fail [ "RPAREN" ]
  ...
  (*
    ...
    ACTION:
      ID TID CODE DWHEN DPREC COMMA PLUS QMARK SEMI STAR RPAREN -> reduce 0 0
  *)
  and state_71 ~_loc a1_separated_nonempty_list _c0_args = 
    match lookahead () with
    (* Reduce *)
    | ID _ | TID _ | CODE _ | DWHEN | DPREC | COMMA | PLUS | QMARK | SEMI | STAR | RPAREN ->
      let _loc = loc_reduce ~_loc 3
  ...
  (*
    ...
    ACTION:
      ID TID CODE DWHEN DPREC COMMA PLUS QMARK SEMI STAR RPAREN -> reduce 0 0
  *)
  and state_72 ~_loc a0_args _c0_loption = 
    match lookahead () with
    (* Reduce *)
    | ID _ | TID _ | CODE _ | DWHEN | DPREC | COMMA | PLUS | QMARK | SEMI | STAR | RPAREN ->
      let _loc = loc_reduce ~_loc 1
  ...
  (*
    ...
    ACTION:
      ID TID CODE DWHEN DPREC COMMA PLUS QMARK SEMI STAR RPAREN -> reduce 0 0
  *)
  and state_73 ~_loc a0_loption a1_symbol _c0_actual = 
    match lookahead () with
    (* Reduce *)
    | ID _ | TID _ | CODE _ | DWHEN | DPREC | COMMA | PLUS | QMARK | SEMI | STAR | RPAREN ->
      let _loc = loc_reduce ~_loc 2
  ...
  (*
    ...
    ACTION:
      ...
      ID TID CODE DWHEN DPREC -> reduce 3 1
  *)
  and state_74 ~_loc a0_actual a2_ID _c0_producer _c1_actual = 
    let rec _c2_shorthand ~_loc x = state_61 ~_loc x a0_actual _c1_actual
    and _c3_list ~_loc x = state_75 ~_loc x a0_actual a2_ID _c0_producer in
    match lookahead () with
    (* Shift *)
    | PLUS ->
      let _, _l = shift () in
  ...
  (*
    ...
    ACTION:
      ID TID CODE DWHEN DPREC -> reduce 0 0
  *)
  and state_75 ~_loc a0_list a1_actual a3_ID _c0_producer = 
    match lookahead () with
    (* Reduce *)
    | ID _ | TID _ | CODE _ | DWHEN | DPREC ->
  ...
  *)
  and state_76 ~_loc a0_production _c0_separated_nonempty_list = 
    match lookahead () with
    (* Shift *)
    | BAR ->
      let _, _l = shift () in
      let _loc = loc_shift ~_loc _l in
      state_77 ~_loc a0_production _c0_separated_nonempty_list
    (* Reduce *)
  ...
  (*
    ...
    ACTION:
      ...
      ID TID -> shift
  *)
  and state_77 ~_loc a1_production _c0_separated_nonempty_list = 
    let rec _c1_symbol ~_loc x = state_52 ~_loc x _c4_actual
    and _c2_production ~_loc x = state_76 ~_loc x _c6_separated_nonempty_list
    and _c3_producer ~_loc x = state_54 ~_loc x _c5_list
    and _c4_actual ~_loc x = state_55 ~_loc x _c3_producer _c4_actual
    and _c5_list ~_loc x = state_78 ~_loc x _c2_production
    and _c6_separated_nonempty_list ~_loc x = state_94 ~_loc x a1_production _c0_separated_nonempty_list in
    match lookahead () with
    (* Reduce *)
    | CODE _ | DWHEN | DPREC ->
      let _loc = loc_reduce ~_loc 0
  ...
  *)
  and state_78 ~_loc a0_list _c0_production = 
    let rec _c1_option ~_loc x = state_81 ~_loc x a0_list _c0_production in
    match lookahead () with
    (* Shift *)
    | DPREC ->
      let _, _l = shift () in
      let _loc = loc_shift ~_loc _l in
      state_79 ~_loc _c1_option
    (* Reduce *)
    | CODE _ | DWHEN ->
      let _loc = loc_reduce ~_loc 0
      and x = Actions.a3 ~_loc () in
  ...
  (*
    ...
    ACTION:
      ID TID -> shift
  *)
  and state_79 ~_loc _c0_option = 
    let rec _c1_ident ~_loc x = state_80 ~_loc x _c0_option in
    match lookahead () with
    (* Shift *)
    | ID x ->
      let _, _l = shift () in
  ...
  *)
  and state_80 ~_loc a0_ident _c0_option = 
    match lookahead () with
    (* Reduce *)
    | CODE _ | DWHEN ->
      let _loc = loc_reduce ~_loc 2
  ...
  (*
    ...
    ACTION:
      ...
      DWHEN -> shift
  *)
  and state_81 ~_loc a0_option a1_list _c0_production = 
    let rec _c1_actions ~_loc x = state_87 ~_loc x a0_option a1_list _c0_production
    and _c2_conditional_action ~_loc x = state_88 ~_loc x _c3_nonempty_list _c4_list
    and _c3_nonempty_list ~_loc x = state_91 ~_loc x _c1_actions
    and _c4_list ~_loc x = state_92 ~_loc x _c1_actions in
    match lookahead () with
    (* Reduce *)
    | CODE _ ->
      let _loc = loc_reduce ~_loc 0
    ...
    | DWHEN ->
      let _, _l = shift () in
      let _loc = loc_shift ~_loc _l in
      state_82 ~_loc _c2_conditional_action
    | _ ->
      fail [ "CODE"; "DWHEN" ]
  ...
  (*
    ...
    ACTION:
      ID TID -> shift
  *)
  and state_82 ~_loc _c0_conditional_action = 
    let rec _c1_symbol ~_loc x = state_83 ~_loc x _c0_conditional_action in
    match lookahead () with
    (* Shift *)
    | ID x ->
      let _, _l = shift () in
  ...
  (*
    ...
    ACTION:
      EQ -> shift
  *)
  and state_83 ~_loc a0_symbol _c0_conditional_action = 
    match lookahead () with
    (* Shift *)
    | EQ ->
      let _, _l = shift () in
      let _loc = loc_shift ~_loc _l in
      state_84 ~_loc a0_symbol _c0_conditional_action
    | _ ->
      fail [ "EQ" ]
  ...
  (*
    ...
    ACTION:
      ID TID -> shift
  *)
  and state_84 ~_loc a1_symbol _c0_conditional_action = 
    let rec _c1_symbol ~_loc x = state_85 ~_loc x a1_symbol _c0_conditional_action in
    match lookahead () with
    (* Shift *)
    | ID x ->
      let _, _l = shift () in
  ...
  (*
    ...
    ACTION:
      CODE -> shift
  *)
  and state_85 ~_loc a0_symbol a2_symbol _c0_conditional_action = 
    match lookahead () with
    (* Shift *)
    | CODE x ->
      let _, _l = shift () in
      let _loc = loc_shift ~_loc _l in
      state_86 ~_loc x a0_symbol a2_symbol _c0_conditional_action
    | _ ->
      fail [ "CODE" ]
  ...
  *)
  and state_86 ~_loc a0_CODE a1_symbol a3_symbol _c0_conditional_action = 
    match lookahead () with
    (* Reduce *)
    | ID _ | CODE _ | DWHEN | DINLINE | BAR | SEMI | EOF ->
      ...
      _c0_conditional_action ~_loc x
    | _ ->
      fail [ "ID"; "CODE"; "DWHEN"; "DINLINE"; "BAR"; "SEMI"; "EOF" ]
  ...
  *)
  and state_87 ~_loc a0_actions a1_option a2_list _c0_production = 
    match lookahead () with
    (* Reduce *)
    | ID _ | DINLINE | BAR | SEMI | EOF ->
      let _loc = loc_reduce ~_loc 3
      and x = Actions.a47 ~_loc a0_actions a1_option a2_list () in
      _c0_production ~_loc x
    | _ ->
      fail [ "ID"; "DINLINE"; "BAR"; "SEMI"; "EOF" ]
  ...
  *)
  and state_88 ~_loc a0_conditional_action _c0_nonempty_list _c1_list = 
    let rec _c2_conditional_action ~_loc x = state_88 ~_loc x _c3_nonempty_list _c4_list
    and _c3_nonempty_list ~_loc x = state_89 ~_loc x a0_conditional_action _c0_nonempty_list
    and _c4_list ~_loc x = state_90 ~_loc x a0_conditional_action _c1_list in
    match lookahead () with
    (* Reduce *)
    | CODE _ ->
      let _loc = loc_reduce ~_loc 0
  ...
  (*
    ...
    ACTION:
      ID DINLINE BAR SEMI EOF -> reduce 0 0
  *)
  and state_89 ~_loc a0_nonempty_list a1_conditional_action _c0_nonempty_list = 
    match lookahead () with
    (* Reduce *)
    | ID _ | DINLINE | BAR | SEMI | EOF ->
      let _loc = loc_reduce ~_loc 2
  ...
  (*
    ...
    ACTION:
      CODE -> reduce 0 0
  *)
  and state_90 ~_loc a0_list a1_conditional_action _c0_list = 
    match lookahead () with
    (* Reduce *)
    | CODE _ ->
      let _loc = loc_reduce ~_loc 2
  ...
  (*
    ...
    ACTION:
      ID DINLINE BAR SEMI EOF -> reduce 0 0
  *)
  and state_91 ~_loc a0_nonempty_list _c0_actions = 
    match lookahead () with
    (* Reduce *)
    | ID _ | DINLINE | BAR | SEMI | EOF ->
      let _loc = loc_reduce ~_loc 1
  ...
  (*
    ...
    ACTION:
      CODE -> shift
  *)
  and state_92 ~_loc a0_list _c0_actions = 
    match lookahead () with
    (* Shift *)
    | CODE x ->
      let _, _l = shift () in
      let _loc = loc_shift ~_loc _l in
      state_93 ~_loc x a0_list _c0_actions
    | _ ->
      fail [ "CODE" ]
  ...
  (*
    ...
    ACTION:
      ID DINLINE BAR SEMI EOF -> reduce 0 0
  *)
  and state_93 ~_loc a0_CODE a1_list _c0_actions = 
    match lookahead () with
    (* Reduce *)
    | ID _ | DINLINE | BAR | SEMI | EOF ->
      let _loc = loc_reduce ~_loc 2
  ...
  *)
  and state_94 ~_loc a0_separated_nonempty_list a2_production _c0_separated_nonempty_list = 
    match lookahead () with
    (* Reduce *)
    | ID _ | DINLINE | SEMI | EOF ->
      let _loc = loc_reduce ~_loc 3
  ...
  *)
  and state_95 ~_loc a0_separated_nonempty_list a1_option a3_loption a4_ID a5_boption _c0_rule = 
    let rec _c1_list ~_loc x = state_96 ~_loc x a0_separated_nonempty_list a1_option a3_loption a4_ID a5_boption _c0_rule in
    match lookahead () with
    (* Shift *)
    | SEMI ->
      let _, _l = shift () in
  ...
  *)
  and state_96 ~_loc a0_list a1_separated_nonempty_list a2_option a4_loption a5_ID a6_boption _c0_rule = 
    match lookahead () with
    (* Reduce *)
    | ID _ | DINLINE | EOF ->
  ...
  (*
    ...
    ACTION:
      EOF -> reduce 0 0
  *)
  and state_97 ~_loc a0_list a1_rule _c0_list = 
    match lookahead () with
    (* Reduce *)
    | EOF ->
      let _loc = loc_reduce ~_loc 2
  ...
  (*
    ...
    ACTION:
      EOF -> shift
  *)
  and state_98 ~_loc a0_list a2_list _c0_grammar_starting = 
    match lookahead () with
    (* Shift *)
    | EOF ->
      let _, _l = shift () in
      let _loc = loc_shift ~_loc _l in
      state_99 ~_loc a0_list a2_list _c0_grammar_starting
    | _ ->
      fail [ "EOF" ]
  ...
  (*
    ITEMS:
      grammar' → list DSEP list EOF .
  *)
  and state_99 ~_loc a1_list a3_list _c0_grammar_starting = 
